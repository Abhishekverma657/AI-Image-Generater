...
const Result = () => {
   const {generareImage}=useContext(AppContext)
      ...
      const [loading, setLoading] = useState(false)
       const [input, setInput] = useState('')
        ...
        const onsubmitHandler=async(e)=>{
          ...
          setLoading(true)
          if(input){
            const image=await generareImage(input);
            if(image){
  ...
  return (
    ...
    {!isImageLoaded&& 
     ...
     <input onChange={e=>setInput(e.target.value)} value={input} type="text" placeholder='Describe what you want to generate' className='flex-1 bg-transparent outline-none ml-8 max-sm:w-20 placeholder-white' />
